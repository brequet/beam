...
| --------- | ----------- | --------- | -------------------------------------------- |
| `--host`  | `ZAPPETTE_HOST` | `0.0.0.0` | Address to bind the web server to            |
| `--port`  | `ZAPPETTE_PORT` | `5000`    | Port to bind the web server to               |
...
ZAPPETTE_PORT=8080 cargo run          # env fallback
```

### Run at logon (Windows)

```sh
zappette install                      # autostart with default options
zappette install --port 8080 --mock   # flags passed here are frozen into the task
zappette uninstall                    # remove the task (stops a task-started zappette)
```

`zappette install` registers a Task Scheduler task with an *at log on* trigger
...
`zappette --hidden`: no console window, and the current URL is written to
`%LOCALAPPDATA%\zappette\zappette.log`. The task never gets killed by the scheduler's
...
away without waiting for the next logon: `schtasks /run /tn zappette`.
